...
function renderFinish(jumpsA) {
    ...
    displayFinishModal(daily_idx, totalJumps, newStreak, true)
}
...
function generateEmojiGrid(scores) {
    const columns = scores.map(score => {
        let color;
        if (score === 2) color = "⬜";
        else if (score === 3) color = "🟩";
        else if (score === 4 || score === 5) color = "🟨";
        else color = "🟥";
        const height = Math.min(score >= 6 ? 6 : score, 6);
        const col = Array(height).fill(color);
        while (col.length < 6) col.unshift("⬛");
        return col;
    });
...
function displayFinishModal(daily_idx, totalJumps, currentStreak, is_user=false) {
    ...
    if (is_user){
        ...
        emojiGrid = generateEmojiGrid(jumpsArray);
        modalFinish.querySelector('.jumpsA').textContent = emojiGrid;
    }
}
